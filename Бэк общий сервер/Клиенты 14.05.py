...
def create_user_database(database_name, num_users=50):
    conn = sqlite3.connect(database_name)
    c = conn.cursor()

    c.execute('''CREATE TABLE IF NOT EXISTS users
                 (id INTEGER PRIMARY KEY, fullname TEXT, username TEXT, password TEXT, street TEXT, city TEXT, postcode TEXT, email TEXT, 
                 school TEXT, school_start_year INTEGER, school_end_year INTEGER, 
                 college TEXT, college_start_year INTEGER, college_end_year INTEGER, 
                 university TEXT, university_start_year INTEGER, university_end_year INTEGER)''')
    ...
    for _ in range(num_users):
        ...
        school_start_year = random.randint(2000, 2015)
        school_end_year = random.randint(school_start_year, 2020)
        college = random.choice(colleges)
        college_start_year = random.randint(2010, 2015)
        college_end_year = random.randint(college_start_year, 2020)
        university = random.choice(universities)
        university_start_year = random.randint(2015, 2020)
        university_end_year = random.randint(university_start_year, 2024)
        c.execute("INSERT INTO users (fullname, username, password, street, city, postcode, email, "
    ...
    os.rename(database_name, 'clients.sqlite3')
