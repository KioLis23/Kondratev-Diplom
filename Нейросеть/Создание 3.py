...
# Вычисление MSE для каждого примера
mse = tf.keras.losses.MeanSquaredError()
mse_values = np.array([mse(X_test[i], predictions[i]).numpy() for i in range(len(X_test))])

# Порог для обнаружения аномалий
threshold = 0.025  # Порог можно настроить под вашу задачу

# Обнаружение атак
anomalies = (mse_values > threshold).astype(int)

print("Метрика MSE:", mse_values)
...
plt.subplot(1, 2, 2)
plt.scatter(range(len(mse_values)), mse_values, c=anomalies, cmap='coolwarm', s=50)
